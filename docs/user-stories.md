...
## US-003bis — Générer listes et étiquettes au format Excel (template ALPE)

```yaml
id: US-003bis
title: Générer listes et étiquettes au format Excel compatible template ALPE
actor: gestionnaire
benefit: "...pour maintenir la compatibilité avec le processus actuel et faciliter la transition"
as_a: "En tant que gestionnaire"
i_want: "Je veux générer des fichiers Excel par déposant conformes au template actuel (ListeALPEAvecMacro_Aut 25.xlsm) mais enrichis avec codes uniques et QR codes"
so_that: "Afin de permettre une transition en douceur depuis le processus Google Form + Excel VBA vers l'application, tout en ajoutant le scannage en caisse"

# Contexte métier
notes: |
  - Cette US complète la US-003 (génération PDF) en offrant une alternative Excel
  - **Template actuel** : docs/ListeALPEAvecMacro_Aut 25.xlsm (voir docs/template-excel-structure.md)
  - Structure actuelle : 4 onglets (ExportForm, Liste, Etiqu12, Etiqu24)
  - **Processus actuel** :
    • Déposant remplit Google Form
    • Gestionnaire copie ligne depuis Google Sheet vers ExportForm
    • Macro VBA génère Liste + Étiquettes
    • Impression et découpage manuel
  - **Évolution proposée** :
    • Remplacer Google Form par saisie app (US-002)
    • Générer Excel automatiquement (US-003bis)
    • Ajouter QR codes sur étiquettes (nouveau)
    • Conserver format familier pour bénévoles (transition douce)
  - Les bénévoles sont habitués au format Excel actuel
  - Export Excel permet impression depuis n'importe quel poste
  - Alternative au PDF pour ceux qui préfèrent Excel

acceptance_criteria:
  # AC-1 : Option de génération Excel en complément PDF
  - GIVEN je suis sur la page "Gestion des étiquettes"
    AND j'ai sélectionné un ou plusieurs déposants
    WHEN je clique sur "Générer"
    THEN je vois deux options de format :
      • "📄 PDF (recommandé)" - génération selon US-003
      • "📊 Excel (format ALPE)" - génération compatible template actuel
    AND je peux choisir le format souhaité avant génération

  # AC-2 : Génération fichier Excel individuel - structure
  - GIVEN je génère le fichier Excel pour le déposant "Marie Dupont" (Liste 245, 18 articles)
    WHEN le système crée le fichier Excel
    THEN le fichier nommé "Liste_245_Marie_Dupont.xlsx" contient 3 onglets :
      • Onglet 1 : "Liste" - liste d'articles du déposant
      • Onglet 2 : "Etiquettes" - planche d'étiquettes (18 étiquettes)
      • Onglet 3 : "Codes" - tableau de référence codes uniques/QR codes
    AND le fichier est au format .xlsx (pas de macros VBA nécessaires)

  # AC-3 : Onglet "Liste" - format conforme au template
  - GIVEN l'onglet "Liste" du fichier Excel généré
    THEN il contient le format du template actuel :
      • En-tête :
      ...
      • Bloc info déposant :
        - Nom : Marie Dupont
        - Prénom : Marie
        - Adresse : 12 rue des Fleurs / 31830 Plaisance du Touch
        - Téléphone : 06 12 34 56 78
        - Date : _ _ / _ _ / _ _
      • Tableau des articles (3 colonnes) :
        | N° | Désignation | Prix | Vendu/NV |
        |  1 | Pull rayé bleu marine | 5.00€ | □ |
        |    | Vêtements - Garçon - 4 ans |      |   |
        |    | Bleu marine - H&M |      |   |
      • Sous-colonnes dans "Désignation" :
        - Ligne 1 : Description article
        - Ligne 2 : Catégorie - Genre - Taille
        - Ligne 3 : Couleur - Marque
      • Colonne "Vendu/NV" : case à cocher vide (pour usage manuel)
      • Pied de page : champs bénévoles, enveloppe timbrée, signature

  # AC-4 : Onglet "Liste" - enrichissements nouveaux
  - GIVEN l'onglet "Liste" généré
    THEN il contient en plus du template actuel :
      • Colonne "Code" (cachée par défaut, peut être affichée) contenant EDI-xxx-L245-A01, etc.
      • Ligne de total : "Total : 18 articles - 87.00€"
      • Zone "QR Code liste" en haut à droite (miniature QR du numéro liste)
      • Dates pré-remplies : date génération, créneau dépôt
      • Note en bas : "Ce document a été généré le [DATE] à [HEURE] par [GESTIONNAIRE]"

  # AC-5 : Onglet "Etiquettes" - format adapté Excel
  - GIVEN l'onglet "Etiquettes"
    THEN il contient une planche d'étiquettes mise en page pour impression :
      • Disposition : 2 colonnes × 9 lignes (18 étiquettes sur 2 pages A4)
      • Chaque étiquette (dimensions approximatives en cellules Excel) :
        - QR code : image insérée dans cellule fusionnée (3×3 cellules)
        - Texte "Liste 245"
        - Texte "Article 3/18"
        - Prix : "5.00€" en gros
        - Description : "Pull rayé bleu..." (30 car max)
        - Catégorie : "Vêtements"
        - Texte "ATTENTION : Conserver pour la restitution"
        - Texte "Restitution : Lundi 18h30-19h30"
        - Code unique (petit) : "EDI-2024-11-L245-A03"
      • Lignes pointillées entre étiquettes (bordures en pointillés)
      • Mise en page : marges 1cm, orientation portrait
      • Couleur de fond selon numéro liste (ex: Liste 245 → jaune soleil)

  # AC-6 : Onglet "Codes" - référence technique
  - GIVEN l'onglet "Codes"
    THEN il contient un tableau de référence :
      | N° | Description | Prix | Code unique | QR Code | Vendu |
      |  1 | Pull rayé... | 5€ | EDI-2024-11-L245-A01 | [Image QR] | Non |
      |  2 | Pantalon... | 7€ | EDI-2024-11-L245-A02 | [Image QR] | Non |
      | ... | ... | ... | ... | ... | ... |
    AND colonne "QR Code" contient les images QR générées (30×30 pixels)
    AND colonne "Vendu" est initialisée à "Non"
    AND cet onglet sert de référence pour scan/import ultérieur

  # AC-7 : Génération images QR code dans Excel
  - GIVEN le système génère le fichier Excel
    WHEN il crée les QR codes
    THEN pour chaque article :
      • Génère image QR code (format PNG, 150×150 pixels)
      • Contenu QR : code unique "EDI-2024-11-L245-A03"
      • Insère image dans cellule onglet "Etiquettes"
      • Insère image miniature dans cellule onglet "Codes"
      • Images intégrées au fichier Excel (pas de fichiers externes)

  # AC-8 : Compatibilité impression
  - GIVEN le fichier Excel généré
    WHEN le gestionnaire l'ouvre dans Excel/LibreOffice
    THEN :
      • Onglet "Liste" : zone d'impression définie (1 page A4 portrait)
      • Onglet "Etiquettes" : zone d'impression définie (2 pages A4 portrait pour 18 articles)
      • Marges et en-têtes/pieds de page configurés
      • Bouton "Imprimer" Excel imprime directement les bons onglets
      • Format compatible Excel 2016+, LibreOffice Calc 6+, Google Sheets

  # AC-9 : Génération en masse Excel
  - GIVEN je veux générer les fichiers Excel pour un créneau de 20 déposants
    WHEN je sélectionne "Format Excel" et "Générer pour ce créneau"
    THEN le système :
      • Crée 20 fichiers Excel individuels (un par déposant)
      • Crée un fichier ZIP : "Creneau_Mercredi_9h30_Excel.zip" contenant les 20 fichiers
      • Lance le téléchargement du ZIP
      • Affiche un message : "20 fichiers Excel générés et compressés"
    AND chaque fichier Excel porte le nom "Liste_[NUMERO]_[NOM_PRENOM].xlsx"

  # AC-10 : Export consolidé Excel (optionnel)
  - GIVEN je veux un fichier Excel unique pour tout un créneau
    WHEN je coche "Fichier unique multi-onglets" avant génération
    THEN le système crée un fichier "Creneau_Mercredi_9h30_Consolide.xlsx" avec :
      • Onglet "Sommaire" : liste des 20 déposants avec hyperliens vers leurs onglets
      • Onglets "Liste_245", "Liste_246", etc. : une liste par onglet
      • Onglets "Etiq_245", "Etiq_246", etc. : étiquettes par onglet
      • Onglet "Codes_Global" : tous les codes uniques de tous les déposants
    AND ce format permet d'imprimer toutes les listes d'un coup

  # AC-11 : Prévisualisation avant génération
  - GIVEN je vais générer un fichier Excel
    WHEN je clique sur "Aperçu Excel"
    THEN le système :
      • Génère un fichier Excel de démo pour le premier déposant sélectionné
      • Affiche une capture d'écran des 3 onglets dans une modale
      • Propose "Télécharger l'aperçu" pour vérifier dans Excel
      • Bouton "Confirmer et générer pour tous" lance la génération complète

  # AC-12 : Migration depuis Google Form (bonus)
  - GIVEN l'application remplace le Google Form
    AND des bénévoles souhaitent importer les anciennes données
    WHEN j'accède à "Outils > Import Google Form"
    THEN je peux :
      • Copier-coller une ligne depuis le Google Sheet
      • Le système parse les colonnes (Nom, Prénom, Articles...)
      • Crée automatiquement le déposant et ses articles dans l'app
      • Génère le fichier Excel immédiatement
    AND cela facilite la transition du processus actuel vers l'app

dependencies:
  - US-002  # Saisie articles (source des données)
  - US-003  # Génération PDF (alternative)
  - US-008  # Import Billetweb (pour créneau dépôt)

links:
  - rel: requirement
    id: REQ-F-003  # Génération étiquettes scannables
  - rel: source
    href: docs/ListeALPEAvecMacro_Aut 25.xlsm
    title: Template Excel actuel
  - rel: documentation
    href: docs/template-excel-structure.md
    title: Documentation structure template Excel

# Règles métier complémentaires
business_rules:
  - Format Excel .xlsx (pas de macros VBA, compatibilité maximale)
  - Structure 3 onglets : Liste, Etiquettes, Codes
  - Onglet Liste conforme au template actuel + enrichissements (codes, total, QR miniature)
  - Onglet Etiquettes avec QR codes intégrés comme images PNG
  - Onglet Codes pour référence technique et traçabilité
  - Images QR codes intégrées au fichier Excel (pas de fichiers externes)
  - Zones d'impression pré-configurées pour faciliter impression
  - Génération individuelle (1 fichier par déposant) ou consolidée (1 fichier multi-onglets)
  - Export ZIP pour génération en masse
  - Compatible Excel 2016+, LibreOffice Calc 6+, Google Sheets
  - Couleurs de fond étiquettes selon numéro liste (même logique qu'US-003)
  - Format étiquette adapté à Excel (2 colonnes, cellules fusionnées, images)

# Spécifications techniques Excel
excel_specs:
  - Format : Office Open XML (.xlsx)
  - Compatibilité : Excel 2016+, LibreOffice Calc 6.0+, Google Sheets
  - Onglets : 3 (Liste, Etiquettes, Codes)
  - Images QR : PNG 150×150px, intégrées via balise <drawing>
  - Mise en page :
    - Liste : 1 page A4 portrait, marges 1.5cm
    - Etiquettes : 2 colonnes, marges 1cm, A4 portrait
  - Polices : Arial (disponible partout)
  - Couleurs : palette Web safe (compatibilité impression N&B)
  - Formules : aucune (valeurs statiques pour éviter erreurs)
  - Protection : onglet Codes peut être protégé en lecture seule

# Cas de test suggérés
test_scenarios:
  - T-US003bis-01 : Génération Excel pour 1 déposant 18 articles (OK, 3 onglets, 2 pages étiquettes)
  - T-US003bis-02 : Vérification format onglet Liste conforme template actuel (colonnes, mise en page OK)
  - T-US003bis-03 : Vérification QR codes intégrés onglet Etiquettes (18 images PNG présentes)
  - T-US003bis-04 : Ouverture fichier dans Excel 2016 (OK, pas d'erreur, impression OK)
  - T-US003bis-05 : Ouverture fichier dans LibreOffice Calc (OK, QR codes affichés)
  - T-US003bis-06 : Scan QR code depuis étiquette imprimée (smartphone lit code unique)
  - T-US003bis-07 : Génération en masse 20 déposants format Excel (ZIP 20 fichiers créé)
  - T-US003bis-08 : Génération consolidée créneau (1 fichier, 40 onglets, sommaire avec hyperliens OK)
  - T-US003bis-09 : Aperçu Excel avant génération (capture d'écran affichée, téléchargement démo OK)
  - T-US003bis-10 : Vérification couleur fond étiquettes Liste 245 (jaune soleil correct)
  - T-US003bis-11 : Impression onglet Liste (1 page A4, tous les éléments visibles)
  - T-US003bis-12 : Impression onglet Etiquettes (2 pages, 18 étiquettes alignées, découpage facile)
  - T-US003bis-13 : Import ligne Google Form (parsing colonnes OK, déposant créé, Excel généré)
  - T-US003bis-14 : Vérification onglet Codes (18 lignes, QR miniatures, colonnes complètes)
  - T-US003bis-15 : Compatibilité Google Sheets (upload fichier, affichage OK, QR codes visibles)
```
