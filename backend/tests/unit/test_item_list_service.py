...
def mock_edition():
    ...
    edition.declaration_deadline = datetime.now() + timedelta(days=7)
    return edition
...
class TestCanModifyLists:
    ...
    def test_can_modify_when_registrations_open(self, mock_edition):
        ...
        mock_edition.declaration_deadline = datetime.now() + timedelta(days=7)
    ...
    def test_cannot_modify_when_deadline_passed(self, mock_edition):
        ...
        mock_edition.status = EditionStatus.REGISTRATIONS_OPEN.value
        mock_edition.declaration_deadline = datetime.now() - timedelta(days=1)
